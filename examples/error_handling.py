...
class RobotErrorMonitor:
    ...
    def connect(self):
        ...
        try:
            ...
            print(
                f"Successfully connected to robot / 成功连接到机器人: {self.robot_ip}:{self.dashboard_port}"
            )
            return True
    ...
    def check_errors(self, language="zh_cn"):
        ...
        if not errors:
            print(
                "✅ Robot status normal, no error information / 机器人状态正常，无报警信息"
            )
            return False
    ...
    def monitor_errors(self, interval=5, language="zh_cn"):
        ...
        """
        print(
            f"Start monitoring robot error information (check every {interval} seconds) / 开始监控机器人报警信息（每{interval}秒检查一次）"
        )
